...
class TPCDSTableGen(val spark: SparkSession, scale: Double, path: String) extends Serializable {
  ...
  private def catalogReturnSchema = {
    StructType(Seq(
...
object TPCDSTableGen {
  val SCHEMA_RESOURCE = "tpcds-schema.sch"
  ...
  def main(args: Array[String]): Unit = {
    val text = FileUtils.readFileToString(new File(locateResourcePath(SCHEMA_RESOURCE)))
    val generatedCode = StringBuilder.newBuilder

    // tokens
    val TABLE_NAME = "\"\\w+\"".r
    val COLUMN_NAME = "\\w+".r
    val DATA_TYPE = "\\.\\w+".r
    val DECIMAL_SPEC = "decimal\\(\\d+\\)".r

    val tokenizer = new StringTokenizer(text)
    val tokens = new util.LinkedList[String]()
    ...
    println(generatedCode.toString())

    def lookahead(): String = tokens.peek()
    def consume(): Unit = tokens.pop()
    def nextToken(): String = tokens.pop()
    ...
    def table() = {
      consume()
      val token = nextToken()
      val casePrettier = CaseFormat.LOWER_UNDERSCORE.converterTo(CaseFormat.LOWER_CAMEL)
      val tableName = casePrettier.convert(token.substring(1, token.length - 2))
      generatedCode
          .append("private def %sSchema = {".format(tableName))
          .append(System.lineSeparator())
          .append("StructType(Seq(")
          .append(System.lineSeparator())
      breakable {
        var first = true
        while (true) {
    ...
    def column() = {
      val columnName = nextToken()
      val columnType = nextToken().substring(1)
      println("column: %s, type: %s".format(columnName, columnType))
      val formattedType: String = columnType match {
        case "string" =>
          "StringType"
        case "int" | "long" =>
          "LongType"
      ...
      generatedCode.append("StructField(\"%s\", %s)".format(columnName, formattedType))
    }
  }
}
